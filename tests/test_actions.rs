use openwhisk_rust::{WskProperties, NativeClient, Action,Exec, KeyValue};
use std::{fs, io::Read};
...
fn test_create_action(){
    ...
    let mut contents = fs::File::open("/home/soul/Downloads/actions/car_list.zip").expect("File not found");
    ...
    let action = Action{ 
        ...
        publish: true, 
        updated: 0, 
        annotations: vec![KeyValue{ key: "feed".to_string(), value: serde_json::json!({}) }],
    };

    let actions = client.actions().insert(&action, true).unwrap();
